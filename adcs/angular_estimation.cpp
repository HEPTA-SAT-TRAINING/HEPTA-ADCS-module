...
void AngularEstimation::update(Bno055 &sensor, unsigned long now_ms) {
  ...
  if (has_previous_sample_) {
    //荳谺｡繝ｭ繝ｼ繝代せ繝輔ぅ繝ｫ繧ｿ縺ｧ繧ｸ繝｣繧､繝ｭ縺ｮ繝弱う繧ｺ繧剃ｽ取ｸ帙☆繧九ゅヵ繧｣繝ｫ繧ｿ菫よ焚縺ｯGYRO_FILTER_ALPHA縺ｧ隱ｿ謨ｴ縺吶ｋ縲・
    ...
    if (mode_ != ESTIMATION_MAG_ONLY &&
        sample_interval_ms <= MAX_GYRO_INTEGRATION_INTERVAL_MS) {
  ...
  //逎∵ｰ励そ繝ｳ繧ｵ縺ｮ蛟､繧貞叙蠕励＠・悟慍逎∵ｰ・蝓ｺ貅・縺ｨ繝ｪ繧｢繝ｫ繧ｿ繧､繝縺ｮ蝨ｰ逎∵ｰ励・蜷代″縺九ｉ・後Κ繝ｼ隗偵ｒ邂怜・
  //繧ｸ繝｣繧､繝ｭ繧ｻ繝ｳ繧ｵ縺ｮ蛟､縺ｫ・悟慍逎∵ｰ励Κ繝ｼ隗偵・隱､蟾ｮ繧定｣懈ｭ｣
  float mag_x_ut;
  float mag_y_ut;
  float mag_z_ut;
  ...
  // Gyro-only mode still samples the magnetometer for telemetry, but never
  // lets it alter the attitude used by the controller.
  if (mode_ == ESTIMATION_GYRO_ONLY) {
    magnetometer_is_valid_ = true;
    return;
  }
...
void AngularEstimation::set_mode(EstimationMode mode, unsigned long now_ms) {
  mode_ = mode;
  reset(now_ms);
}

AngularEstimation::EstimationMode AngularEstimation::mode() const {
  return mode_;
}

const char *AngularEstimation::mode_name() const {
  switch (mode_) {
    case ESTIMATION_GYRO_ONLY:
      return "GYRO";
    case ESTIMATION_MAG_ONLY:
      return "MAG";
    case ESTIMATION_FUSION:
      return "FUSION";
    default:
      return "UNKNOWN";
  }
